...
app.post("/fases/progresso/:faseId", authenticateToken, (req, res) => {
    ...
    if (faseId > 5 || faseId < 1) {
        res.status(400).json({ message: "Essa fase não existe" })
    }
        ...
        db.run(sql, [userId, faseId], function (err) {
            ...
            res.status(200).json({ message: `Progresso da fase ${faseId} salvo!`, userId, faseId });
        });
    }
});
...
app.delete("/auth/user/:username", (req, res) => {
...
app.delete("/fases/progresso/:userId", (req, res) => {
  const userId = req.params.userId;
  const sql = `DELETE FROM progress WHERE user_id = ?`;

  db.run(sql, [userId], function (err) {
    if (err) return res.status(500).json({ message: "Erro ao deletar progresso" });

    res.json({
      message: "Progresso deletado",
      deletedRows: this.changes // quantidade de linhas apagadas
    });
  });
});
